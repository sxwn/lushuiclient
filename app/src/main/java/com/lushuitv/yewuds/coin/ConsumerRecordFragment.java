...
public class ConsumerRecordFragment extends Fragment {
    ...
    private List<CoinListResponse.OBJECTBean> consumeRecords;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_consumer_record,container,false);
        initView(view);
        initData();
        return view;
    }
    private void initView(View view) {
        consumeRecords = new ArrayList<>();
        mRecyclerView = (EmptyRecyclerView) view.findViewById(R.id.fragment_consume_record_recycleview);
        ...
        adapter = new RecordAdapter(getActivity(),consumeRecords);
        ...
        //监听上拉加载更多
        mRecyclerView.addOnScrollListener(new ConsumerRecordFragment.RecyclerViewScrollListener());
    }
    /**
     * RecyclerView 滑动监听器
     */
    ...
    private void getDataFromServer(int type) {
        ...
        ApiRetrofit.getInstance().getApiService().getConsumeRecord(version,userId,mPage,Constants.PAGE_SIZE)
                ...
                .subscribe(new Observer<CoinListResponse>() {
                    ...
                    public void onNext(CoinListResponse coinListResponse) {
                        ...
                        if (Constants.GET_DATA_TYPE_NORMAL == type) {
                            //正常模式下，清空之前数据，重新加载
                            consumeRecords.clear();
                            consumeRecords = coinListResponse.getOBJECT();
                        } else {
                            //加载更多模式
                            consumeRecords.addAll(coinListResponse.getOBJECT());
                        }
                        //如果获取的数据不足一页，代表当前已经没有更过数据，关闭加载更多
                        if (coinListResponse.getOBJECT().size() < Constants.PAGE_SIZE) {
                            mIsLoadMore = false;
                        }
                        adapter.setData(consumeRecords);
    ...
    private void initData() {
        String version = SystemUtils.getVersion(getActivity());
        String userId = Config.getCachedUserId(getActivity());
        ApiRetrofit.getInstance().getApiService().getConsumeRecord(version,userId,1,9)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<CoinListResponse>(){
