...
import com.lushuitv.yewuds.utils.DensityUtil;
...
public class PasswordInputView extends EditText {
    // 默认外边缘线条宽度为1dp
    private final float DEFAULT_OUT_LINE_WIDTH = DensityUtil.dip2px(getContext(), 1.0f);
    // 默认外边缘线条颜色
    private final int DEFAULT_OUT_LINE_COLOR = Color.parseColor("#bebebe");
    // 默认外边缘弧度
    private final float DEFAULT_OUT_RADIUS = 0f;
    // 默认间隔线宽度
    private final float DEFAULT_DIVIDER_LINE_WIDTH = DensityUtil.dip2px(getContext(),1.0f);
    // 默认间隔线颜色
    private final int DEFAULT_DIVIDER_LINE_COLOR = Color.parseColor("#eeeeee");
    // 默认占位圆点半径
    private final float DEFAULT_DOT_RADIUS = DensityUtil.dip2px(getContext(),7.5f);
    // 默认占位圆点颜色
    private final int DEFAULT_DOT_COLOR = Color.BLACK;
    // 默认输入框宽度
    private final float DEFAULT_INPUT_BOX_WIDTH = DensityUtil.dip2px(getContext(),30f);
    // 默认密码长度
